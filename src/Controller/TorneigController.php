...
class TorneigController extends AbstractController
{
     ...
     * @Route("/{id}", name="torneig_delete", methods={"DELETE"})
     */
    public function ficarResultatPartida(Request $request, Torneig $torneig): Response
    {
        $idPartida = $request->get("idPartida");
        $guanyador = $request->get("guanyador");
        $rondes = $torneig->getRondes();
        $numRondes = count($rondes);
        $ronda = $rondes[$numRondes-1];
        $partides = $ronda->getPartides();
        ...
        if($guanyador == "blanques"){
            $partida->setGuanyador(true);
        }else if($guanyador == "negres"){
            $partida->setGuanyador(false);
        }else{
